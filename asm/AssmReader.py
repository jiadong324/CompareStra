...
def split_pav_vcf(workdir, datasets, assemblers, max_size, sv_counts_info, sv_counts_by_regions,  simple_reps, rmsk, sds, exclude_dict, cmrg_dict, passed_dict):
    ...
    for assembler in assemblers:
        for dataset in datasets:
            ...
            svtype_num = {}
            svtype_by_region = {region: {SVTYPEMAP[svtype]: 0 for svtype in svtypes} for region in region_labels}
...
def split_svimasm_vcf(workdir, datasets, assemblers, max_size, sv_counts_info, sv_counts_by_regions, simple_reps, rmsk, sds, exclude_dict, cmrg_dict, passed_dict):
    ...
    for assembler in assemblers:
        for dataset in datasets:
            ...
            sv_in_cmrg = open(f'{current_dir}/{aligner}_{dataset}/filtered_in_cmrg/HG002.svimasm.{assembler}.vcf', 'w')
            sv_in_passed = open(f'{current_dir}/{aligner}_{dataset}/filtered_in_highconf/HG002.svimasm.{assembler}.vcf', 'w')
...
'''
def process_pav_bed(workdir, datasets, max_size, simrep_path, rmsk_path, exclude_path):
    simple_reps = pysam.Tabixfile(simrep_path, 'r')
    rmsk = pysam.Tabixfile(rmsk_path, 'r')
    exclude_dict = parse_exclude_regions(exclude_path)
    ...
    for dataset in datasets:
        pav_total = 0
        sv_count_writer = open(f'{workdir}/{dataset}/pav_sv_counts.tsv', 'w')
        for svtype in svtypes:
            pav_bed = f'{workdir}/{dataset}/sv_{svtype}.bed'
            pav_annot = f'{workdir}/{dataset}/sv_{svtype}_annot.bed'

            sv_count = annotate_pav_calls(pav_bed, pav_annot, max_size, simple_reps, rmsk, exclude_dict)
            pav_total += sv_count
            print(f'{SVTYPEMAP[svtype]}\t{sv_count}', file=sv_count_writer)

        print(f'Total\t{pav_total}', file=sv_count_writer)

        sv_count_writer.close()

def annotate_pav_calls(pav_bed_input, pav_annot_out, max_size, simple_reps, rmsk, exclude_dict):

    out_writer = open(pav_annot_out, 'w')
    df_pavs = pd.read_csv(pav_bed_input, sep='\t', header=[0])
    counter = 0
    for idx, row in df_pavs.iterrows():
        chrom, start, end, svtype, svlen = row['#CHROM'], int(row['POS']), int(row['END']), row['SVTYPE'], int(row['SVLEN'])
        ...
        print(f'{chrom}\t{start}\t{end}\t{svtype}\t{svlen}\t{rptype}\t{round(pcrt, 2)}', file=out_writer)

    print(f'# SVs: {len(df_pavs)}, after filter: {counter}')
    return counter  
'''