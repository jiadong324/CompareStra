...
def process_read_calls(workdir, datasets, aligners, max_size):
    exclude_dict = parse_exclude_regions(iMACEXCLUDE)
    cmrg_regions = parse_exclude_regions(CMRG)
    passed_regions = parse_exclude_regions(HIGHCONF)


    ref_file = pysam.FastaFile(iMACREF)
    ...
    for caller in CALLERS:
        for dataset in datasets:
            ...
            for aligner in aligners:
                filtered_dir = f'{current_dir}/{aligner}_{dataset}/filtered'
                if not os.path.exists(filtered_dir):
                    os.mkdir(filtered_dir)

                vcf_path = f'{current_dir}/{aligner}_{dataset}/raw_calls/HG002.{caller}.s5.vcf'
                filtered_vcf_path = f'{filtered_dir}/HG002.{caller}.filtered.vcf'
                ...
                high_conf_dir = f'{current_dir}/{aligner}_{dataset}/filtered_in_highconf'
                ...
                svs_in_region.append((caller, dataset, aligner, 'PASS', passed_svs))
    ...
    df_svregions.to_csv(f'{workdir}/caller_sv_in_cmrg_passed.tsv', sep='\t', header=True, index=False)
...
def split_svtypes_to_files(input_vcf, noalt_vcf, caller, minsr, max_size, exclude_dict, ref_file, output_dir, output_prefix, simrep, rmsk, sds):
    ...
    svtype_by_region = {region: {svtype: 0 for svtype in svtypes} for region in region_labels}
    ...
    with open(input_vcf, 'r') as f:
        for line in f:
            ...
            if sv_type in ['INS', 'DEL', 'INV', 'DUP']:
                ...
                region, rptype, pcrt = annotate_sv_region(chrom, start, end, 50, simrep, rmsk, sds)
...
def annotate_read_highconf_svs(workdir, datasets, aligners):
    ...
    for col_idx, aligner in enumerate(aligners):
        for dataset in datasets:
            ...
            for caller in CALLERS:
                ...
                vcf_file = f'{workdir}/{plat}/{aligner}_{dataset}/filtered_in_highconf/HG002.{caller}.vcf'
                annot_file = open(f'{workdir}/{plat}/{aligner}_{dataset}/filtered_in_highconf/HG002.{caller}.annot.info.tsv', 'w')
                ...
                with open(vcf_file, 'r') as f:
                    for line in f:
                        ...
                        print(f'{chrom}\t{start}\t{end}\t{sv_id}\t{sv_len}\t{sv_type}\t{region}\t{rptype}\t{pcrt}', file=annot_file)
